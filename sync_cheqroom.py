...
def post_with_retry(url, headers, data, timeout=30, max_attempts=4):
    """
    Cheqroom (or something in front of it) occasionally resets the connection
    mid-request with no server-side error — a plain requests.post() treats
    that as fatal and kills the whole sync. Retry a few times with backoff
    before giving up, since a fresh connection attempt usually just works.
    """
    last_err = None
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            print(
                f"Request failed ({err.__class__.__name__}: {err}); "
...
def fetch_list(api_key, user_id, collection, fields):
    ...
    while True:
        ...
        resp = post_with_retry(url, headers=headers, data=data, timeout=30)
        ...
        if len(page_items) < page_size:
            break  # last page
        skip += page_size
        time.sleep(0.3)  # brief pacing between pages — cheap insurance against rate limiting/resets

    return all_items


RESERVATION_FIELDS = "name,status,label,number,fromDate,toDate,customer.name,itemSummary,location.name,fields"
ORDER_FIELDS = "name,status,label,number,started,due,customer.name,itemSummary,location.name,fields"
...
def normalize(raw_items):
    ...
    for r in raw_items:
        ...
        raw_grade = get_field_case_insensitive(r.get("fields") or {}, GRADE_LEVEL_FIELD)
        grade_level = ", ".join(str(v) for v in raw_grade) if isinstance(raw_grade, list) else (raw_grade or "")
        normalized.append(
            {
                "cheqroomId": r.get("_id") or r.get("id"),
                "reservationNumber": r.get("number", ""),
                "name": short_name(full_name),
                "orderName": r.get("name", ""),
                "gradeLevel": grade_level,
                "approvalLabel": r.get("label", ""),
                "date": date_str[:10],
...
def main():
    ...
    windowed = [r for r in expanded if within_window(r["_date"]) or is_overdue_open_checkin(r)]
    overdue_count = sum(1 for r in windowed if is_overdue_open_checkin(r) and not within_window(r["_date"]))
    print(f"Combined items within the 8-week window: {len(windowed)} (including {overdue_count} overdue-but-still-open check-in(s) kept regardless of age)")
    for r in windowed:
        cust = (r.get("customer") or {}).get("name", "?")
        print(f"  - [{r['_source']}/{r['_action']}] {cust} | name={r.get('name')!r} | label={r.get('label')!r} | date={r['_date']} | status={r.get('status')} | fields={r.get('fields')}")

    # Normally only "open" (finalized) items matter for day-to-day workflow.
    # Exception: a "creating" (still-draft) PROFESSOR order happening today or
    # tomorrow is worth showing anyway, since staff may need to start prepping
    # before it's fully locked in. Student drafts stay hidden regardless of
    # date — they're much more likely to still change, and there are too many
    # of them (trainee test orders, in-progress bookings) to be useful to show.
    today = datetime.now(timezone.utc).date()
    tomorrow = today + timedelta(days=1)
    near_term_dates = {today.isoformat(), tomorrow.isoformat()}

    def is_professor(r):
        raw = get_field_case_insensitive(r.get("fields") or {}, GRADE_LEVEL_FIELD)
        grade = ", ".join(str(v) for v in raw) if isinstance(raw, list) else (raw or "")
        return "professor" in grade.lower()

    def is_active(r):
        status = r.get("status")
        if status == "open":
            return True
        if status == "creating" and r["_date"][:10] in near_term_dates and is_professor(r):
            return True
        return False

    active = [r for r in windowed if is_active(r)]
    print(f"Active items (open, plus near-term professor drafts): {len(active)}")
